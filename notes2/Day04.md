...
![image-20240415172620631](assets/Day04/image-20240415172620631.png)
...
![image-20240415172655476](assets/Day04/image-20240415172655476.png)

- 二者区别

![image-20240415172843138](assets/Day04/image-20240415172843138.png)
...
![image-20240415173022349](assets/Day04/image-20240415173022349.png)

- 对话模板：准备Json数据后，自动组装

![image-20240415173057863](assets/Day04/image-20240415173057863.png)

![image-20240415173241106](assets/Day04/image-20240415173241106.png)

![image-20240415173250299](assets/Day04/image-20240415173250299.png)
...
![image-20240415173345817](assets/Day04/image-20240415173345817.png)

- Full Finetuning  vs LoRA  vs QLoRA

![image-20240415173513079](assets/Day04/image-20240415173513079.png)
...
![image-20240415173537203](assets/Day04/image-20240415173537203.png)

- 多种生态、多种硬件

![image-20240415173601922](assets/Day04/image-20240415173601922.png)

### 环境配置

![image-20240415173948479](assets/Day04/image-20240415173948479.png)

### 模板选择

- 自定义训练

![image-20240415174028150](assets/Day04/image-20240415174028150.png)

- 对话

![image-20240415174153516](assets/Day04/image-20240415174153516.png)

### Xtuner数据引擎

- 统一处理数据格式：数据预处理

![image-20240415174308306](assets/Day04/image-20240415174308306.png)

- 多数据拼接：并行

![image-20240415174411862](assets/Day04/image-20240415174411862.png)

- 加速方式：Flash Attention（自动开启）、DeepSpeed ZeRO

![image-20240415174513336](assets/Day04/image-20240415174513336.png)
...
![image-20240415174633290](assets/Day04/image-20240415174633290.png)
...
![image-20240415174852542](assets/Day04/image-20240415174852542.png)
...
![image-20240415174947614](assets/Day04/image-20240415174947614.png)
...
![image-20240415175147866](assets/Day04/image-20240415175147866.png)
...
- 让模型能够让模型认清自己的身份弟位，在微调数据集中大量掺杂这部分的数据。

```bash
# 前半部分是创建一个文件夹，后半部分是进入该文件夹。
mkdir -p /root/demo/ft && cd /root/demo/ft

# 在ft这个文件夹里再创建一个存放数据的data文件夹
mkdir -p /root/demo/ft/data && cd /root/demo/ft/data
```
...
touch /root/demo/ft/data/generate_data.py
```
...
data = [
    {
        "messages": [
            ...
            {
                "role": "assistant",
                "content": "我是{}的安全智能小助手，名唤小煤球，内在是上海AI实验室书生·浦语的1.8B大模型哦".format(name)
            }
        ]
    }
]
...
``` bash
# 确保先进入该文件夹
cd /root/demo/ft/data

# 运行代码
python /root/demo/ft/data/generate_data.py
```
...
cp -r /root/share/new_models/Shanghai_AI_Laboratory/internlm2-chat-1_8b/* /root/demo/ft/model/
```
...
# 创建符号链接
ln -s /root/share/new_models/Shanghai_AI_Laboratory/internlm2-chat-1_8b /root/demo/ft/model
```
...
> 输入后我们就能够看到在我们的 `/root/demo/ft/config` 文件夹下有一个名为 `internlm2_1_8b_qlora_alpaca_e3_copy.py` 的文件了。
...
+ pretrained_model_name_or_path = '/root/demo/ft/model'
...
我们可以通过添加 `--work-dir` 指定特定的文件保存位置，比如说就保存在 `/root/demo/ft/train` 路径下。假如不添加的话模型训练的过程文件将默认保存在 `./work_dirs/internlm2_1_8b_qlora_alpaca_e3_copy` 的位置，就比如说我是在 `/root/ft/train` 的路径下输入该指令，那么我的文件保存的位置就是在 `/root/demo/ft/train/work_dirs/internlm2_1_8b_qlora_alpaca_e3_copy` 的位置下。

```bash
# 指定保存路径
xtuner train /root/demo/ft/config/internlm2_1_8b_qlora_alpaca_e3_copy.py --work-dir /root/demo/ft/train
```
...
![image-20240415195824538](assets/Day04/image-20240415195824538.png)

![image-20240415195920304](assets/Day04/image-20240415195920304.png)

![image-20240415195934487](assets/Day04/image-20240415195934487.png)
...
mkdir -p /root/demo/ft/huggingface
...
![image-20240415221539963](assets/Day04/image-20240415221539963.png)
...
```bash
xtuner convert pth_to_hf /root/demo/ft/train/internlm2_1_8b_qlora_alpaca_e3_copy.py /root/demo/ft/train/iter_768.pth /root/demo/ft/huggingface --fp32 --max-shard-size 2GB
```
...
mkdir -p /root/demo/ft/final_model
...
xtuner convert merge /root/demo/ft/model /root/demo/ft/huggingface /root/demo/ft/final_model
```
...
xtuner chat /root/demo/ft/model --adapter /root/demo/ft/huggingface --prompt-template internlm2_chat
```
...
```shell
# 创建存放 InternLM 文件的代码
mkdir -p /root/demo/ft/web_demo && cd /root/demo/ft/web_demo

# 拉取 InternLM 源文件
git clone https://github.com/InternLM/InternLM.git

# 进入该库中
cd /root/demo/ft/web_demo/InternLM
```
...
![image-20240415175434014](assets/Day04/image-20240415175434014.png)
